...
export default function AddTaskScreen({ navigation }: any) {
  ...
  const handleAdd = () => {
    ...
    const newTask: Task = {
      id: `t${Date.now()}`, title: title.trim(), description: description.trim(),
      subject, deadline: parsed.toISOString(), status: 'pending', priority, type,
    };
  ...
  const ChipGroup = ({ label, options, value, onChange }: any) => (
    <View style={{ marginBottom: 18 }}>
      ...
      <View style={styles.chipRow}>
        {options.map((opt: any) => (
          <TouchableOpacity key={opt.value} style={[styles.chip, { backgroundColor: value === opt.value ? opt.color : theme.surfaceAlt, borderColor: value === opt.value ? opt.color : theme.border }]} onPress={() => onChange(opt.value)}>
            {opt.icon && <Ionicons name={opt.icon} size={14} color={value === opt.value ? '#fff' : theme.textSecondary} />}
            <Text style={[styles.chipText, { color: value === opt.value ? '#fff' : theme.textSecondary }]}>{opt.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={{ padding: 16, paddingBottom: 80 }}>
      <View style={[styles.card, { backgroundColor: theme.surface }]}>
        ...
        <TextInput style={[styles.input, { color: theme.text, borderColor: theme.border, backgroundColor: theme.surfaceAlt }]} placeholder="e.g., Midterm Project" placeholderTextColor={theme.textSecondary} value={title} onChangeText={setTitle} />

        <Text style={[styles.label, { color: theme.textSecondary, marginTop: 14 }]}>Description</Text>
        <TextInput style={[styles.textArea, { color: theme.text, borderColor: theme.border, backgroundColor: theme.surfaceAlt }]} placeholder="Optional details..." placeholderTextColor={theme.textSecondary} value={description} onChangeText={setDescription} multiline numberOfLines={3} textAlignVertical="top" />

        <Text style={[styles.label, { color: theme.textSecondary, marginTop: 14 }]}>Subject *</Text>
        <TouchableOpacity style={[styles.picker, { borderColor: theme.border, backgroundColor: theme.surfaceAlt }]} onPress={() => setShowSubjectPicker(!showSubjectPicker)}>
          <Text style={[{ color: subject ? theme.text : theme.textSecondary, flex: 1 }]} numberOfLines={1}>{subject || 'Select a subject...'}</Text>
        ...
        {showSubjectPicker && (
          <View style={[styles.dropdown, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            {SUBJECTS.map(s => (
              <TouchableOpacity key={s} style={[styles.dropdownItem, { borderBottomColor: theme.border }]} onPress={() => { setSubject(s); setShowSubjectPicker(false); }}>
        ...
        <TextInput style={[styles.input, { color: theme.text, borderColor: theme.border, backgroundColor: theme.surfaceAlt }]} placeholder="e.g., 2025-12-31" placeholderTextColor={theme.textSecondary} value={deadline} onChangeText={setDeadline} keyboardType="numbers-and-punctuation" />

        <ChipGroup label="Priority *" value={priority} onChange={setPriority} options={[
